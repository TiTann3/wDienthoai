...
function addTableProducts() {
    ...
    for (var i = 0; i < list_products.length; i++) {
        ...
        s += `<tr>
            ...
            <td style="width: 40%">
                <a title="Xem chi tiết" target="_blank" href="productDetails.html?` + p.name.split(' ').join('-') + `">` + p.name + `</a>
                <img src="` + p.img + `"></img>
            </td>
...
function addKhungSuaSanPham(masp) {
    ...
    var company = ["Apple", "Samsung", "Oppo", "Xiaomi", "Honor", "Realme","Vivo", "Asus", "Masstel", "Nokia"];
        ...
        <tr>
            ...
            <td>
                <select>
                    ...
                    <option value="giare" `+(sp.promo.name == 'giare'?'selected':'')+`>Giá rẻ</option>
    ...
    for(var i = 0; i < u.length; i++) {
        for(var j = 0; j < u[i].donhang.length; j++) {
            ...
            for(var s of u[i].donhang[j].sp) {
                ...
                if(timsp.promo.name == 'giare') tongtien += stringToNum(timsp.promo.value);
